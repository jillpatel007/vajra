...
class VajraConfig(BaseSettings):
    ...
    def clear_secrets_from_env(self) -> None:
        """Remove secret key material from os.environ after loading.

        Why: After pydantic-settings reads VAJRA_HMAC_SECRET_KEY from the
        environment, the value still sits in os.environ. Any code that
        calls os.environ or /proc/self/environ can read it. Clearing it
        reduces the window where the secret is accessible.

        Call this immediately after creating VajraConfig().
        """
